...
HashTable::HashTable() {
    ...
    // Initialize node structure by resizing to the default tableSize
...
void HashTable::Insert(Bid bid) {
    ...
    // if no entry found for the key (empty bucket)
    ...
    // else if the bucket is occupied,
    // then use chaining to attach a new node to the end
    else {
...
void HashTable::PrintAll() {
    ...
    for (unsigned int i = 0; i < nodes.size(); ++i) {
        ...
        if (currentNode->key != UINT_MAX) {
            ...
            // follow the chain if there are more nodes
            Node* chainNode = currentNode->next;
            while (chainNode != nullptr) {
                cout << "Key " << chainNode->key << ": "
                     << chainNode->bid.bidId << " | "
                     << chainNode->bid.title << " | "
                     << chainNode->bid.amount << " | "
                     << chainNode->bid.fund << endl;
...
void HashTable::Remove(string bidId) {
    ...
    // set key equal to hash(atoi(bidId.c_str()))
    ...
    // if key not found in first node, traverse the chain
    while (currentNode != nullptr && currentNode->bid.bidId != bidId) {
        previousNode = currentNode;
        currentNode = currentNode->next;
    }

    // if nothing is found simply return
    ...
    if (previousNode == nullptr) {
        // if there is a chained node, promote it
        if (currentNode->next != nullptr) {
            Node* temp = currentNode->next;
            currentNode->bid = temp->bid;
            currentNode->key = temp->key;
            currentNode->next = temp->next;
            delete temp;
        }
        // otherwise clear the current node
        else {
            currentNode->key = UINT_MAX;
            currentNode->next = nullptr;
        }
    }
    // otherwise unlink it from the chain
    else {
